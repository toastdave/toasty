...
<section class="rounded-[2rem] border border-black/8 bg-white/80 p-8 shadow-[0_24px_90px_-60px_rgba(18,23,34,0.65)] backdrop-blur">
					...
					{#if section.items.length > 0}
						<div class="mt-5 grid gap-4 lg:grid-cols-2">
							{#each section.items as anime (anime.slug)}
								<a class="flex gap-4 rounded-[1.25rem] border border-black/8 bg-white/90 p-4 hover:border-sky-300/70 hover:bg-white" href={`/anime/${anime.slug}`}>
									{#if anime.posterUrl}
	...
	{:else}
		<div class="mt-8 rounded-[1.5rem] border border-dashed border-black/10 bg-cream-50/70 p-6 text-sm leading-7 text-ink-700">
			You have not tracked anything yet. Start on the <a class="font-semibold text-coral-400 hover:text-coral-400/80" href="/anime/top">top anime board</a> or browse the <a class="font-semibold text-coral-400 hover:text-coral-400/80" href="/anime/schedule">current schedule</a> and save a few titles.
		</div>
	{/if}
</section>
