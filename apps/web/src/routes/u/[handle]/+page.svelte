...
<section class="rounded-[2rem] border border-black/8 bg-white/80 p-8 shadow-[0_24px_90px_-60px_rgba(18,23,34,0.65)] backdrop-blur">
	<div class="grid gap-6 lg:grid-cols-[0.78fr_1.22fr]">
		...
		<div>
				...
				{#if data.profile.activity.length > 0}
					<div class="mt-5 space-y-3">
						{#each data.profile.activity as item (item.id)}
							<a class="block rounded-[1.15rem] border border-black/8 bg-white/90 px-4 py-4 hover:border-coral-400/50 hover:bg-white" href={resolveActivityPath(item.href, data.profile.profilePath)}>
			...
			{#if data.profile.recommendationShelf}
				<div class="mt-6 rounded-[1.5rem] border border-black/8 bg-cream-50/80 p-6">
					<div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
						<div class="max-w-3xl">
							<p class="text-sm uppercase tracking-[0.2em] text-ink-700">Likely next picks</p>
							<h2 class="mt-2 text-2xl font-semibold text-ink-950">{data.profile.recommendationShelf.heading}</h2>
							<p class="mt-3 text-sm leading-6 text-ink-700">{data.profile.recommendationShelf.description}</p>
						</div>
					...
					<div class="mt-5 grid gap-4 lg:grid-cols-2">
						{#each data.profile.recommendationShelf.items as anime (anime.slug)}
